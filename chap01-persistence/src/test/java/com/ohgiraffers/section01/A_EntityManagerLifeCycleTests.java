...
public class A_EntityManagerLifeCycleTests {
    ...
    * application 스코프와 동일하게 싱글톤으로 생성해서 관리하는 것이 효율적인다.
    * 따라서 데이터베이스를 사용하는 애플리케이션 당 한 개의 EntityManagerFactory를 생성한다.
    * */

    /*
    * 엔티티 매니저(entityManager)란?
    * 에닡티 매니저는 엔티티를 저장하는 메모리 상의 데이터베이스를 관리하는 인스턴스이다.
    ...
    * 엔티티 매니저를 통해 엔티티를 저장하거나 조회하면 엔티티 매너지는 영속성 컨텍스트에 엔티티를 보관하고 관리한다.
